...
def post_to_yame_prod(yame_id_list):
    ...
    response = requests.post(url='https://sid-metadata.smhi.se/yamr/apipri/merge/test', json = {'uuids': yame_id_list}, headers=headers)
    print(response.text)

if __name__ == "__main__":
    """
    kör detta för att skicka en metadata post i yamr till test 
          curl -X POST -H "Content-Type: application/json" -d '{"uuids": ["9d401958-4726-4a0c-b248-5218995f5e9d"]}'   "https://sid-metadata.smhi.se/yamr/apipri/merge/test"
        hh:55 körs ett jobb i ecFlow som synkar de ändringar man gjort via API'et (eller i Yame) till respektive miljö. Resultatet av denna synkning syns i Yame.
        Alltså, om ni har ändrat en post och skickat denna till test enligt stegen ovan så syns den ändringen på https://sid-metadata-tst.smhi.se/yame/ (med ett ifyllt hänglås eftersom posten inte är skickad till produktion).
    """
