...
class DwcaDataSharkStandard():
    ...
    def create_dwca_keys(self):
        ...
        for row_dict in self.row_list:
            ...
            for _dwc_node_name, dwc_node_dict in self.resources.get_event_nodes().items():
                ...
                dwc_key = dwc_node_dict.get('dwc_key', '')
                ...
                if dwc_key not in self.dwc_short_names_exists_dict:
                    self.dwc_short_names_exists_dict[dwc_key] = {}
                    self.dwc_short_names_exists_dict[dwc_key]['seq_no'] = 0
                    self.dwc_short_names_exists_dict[dwc_key]['short_ids'] = []
                
                if dwc_id not in self.dwc_short_names_exists_dict[dwc_key]['short_ids']:
                    seq_no = self.dwc_short_names_exists_dict[dwc_key]['seq_no']
                    seq_no += 1
                    self.dwc_short_names_exists_dict[dwc_key]['seq_no'] = seq_no
                    self.dwc_short_names_exists_dict[dwc_key]['short_ids'].append(dwc_id)
                #
                seq_no = self.dwc_short_names_exists_dict[dwc_key]['seq_no']
                dwc_id_short = dwc_key_prefix + str(seq_no)
                row_dict[dwc_key] = dwc_id_short
            ...
            # Extra keys for the "measurementorfact.txt" table (aka mof).
    ...
    def get_default_mapping(self):
        ...
        if not self.dwc_default_mapping:
            ...
            for key, value in self.resources.get_field_mapping().items():
                self.dwc_default_mapping[key] = value
        #
